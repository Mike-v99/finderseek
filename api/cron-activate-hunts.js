...
//
// 1. Activates scheduled hunts whose starts_at has passed
// 2. Ends active hunts whose ends_at has passed (no winner)
// 3. Refunds escrow for expired hunts with no winner
// 4. Sends notifications for expired hunts
//
// Env vars needed:
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Fire notifications for newly activated hunts
    for (const hunt of activated) {
      fireNotify('hunt_approved', hunt.id).catch(() => {});
    }

    // ── 2. Find & end expired active hunts (no winner) ──────────
    ...
    for (const hunt of expiredHunts) {
      ...
      // ── 4. Notify pirate that hunt expired ─────────────────────

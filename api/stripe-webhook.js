  ...
  try {
    switch (event.type) {
      ...
      case 'payment_intent.succeeded': {
        ...
        // Mark hunt escrow as funded — hunt can now go active
...
async function verifyStripeSignature(req, secret) {
  ...
  // Manual HMAC verification (avoids needing stripe npm package)
  const parts = sig.split(',').reduce((acc, part) => {
    const [k, v] = part.split('=');
    if (k === 't') acc.timestamp = v;
    if (k === 'v1') acc.signatures.push(v);
...
export default async function handler(req, res) {
  ...
  try {
    switch (event.type) {
      ...
      case 'invoice.payment_failed': {
        // Don't immediately revoke — Stripe retries. Just log.
        console.log(`[webhook] payment_failed for customer ${obj.customer}`);
        break;
      }
    }
